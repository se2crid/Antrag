...
extension HeartbeatManager {
	...
	private func _startHeartbeat(
		...
		Logger.heartbeat.error("we're here 1")
		
		guard inet_pton(AF_INET, remoteIpAddress ?? localIpAddress, &addr.sin_addr) == 1 else {
			Logger.heartbeat.error("Invalid IP address")
			completion(UnknownErrorType)
			return
		}
		
		Logger.heartbeat.error("we're here 2")
		...
		Logger.heartbeat.error("we're here 3")
		...
		Logger.heartbeat.error("we're here 4")
